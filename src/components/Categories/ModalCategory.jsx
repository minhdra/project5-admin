...
import { modalCategoryValidator } from '../../utils/validation';
...
export default function Modal({
  ...
  useEffect(() => {
    if (itemSelected && nameRef) {
      setName(itemSelected.category_name);
      setDescription(itemSelected.description);
      setImageURL(itemSelected.thumbnail);
      pathRef.current.value = itemSelected.name;
    } else emptyValues();
  }, [itemSelected]);
  ...
  const handleSubmit = async (e) => {
    ...
    const validator = modalCategoryValidator(dataPost);
